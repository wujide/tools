...
class File():
    ...
    def setRedundanceFile(self, filepath):
        ...
        try:
            if os.path.isdir(filepath):
                ...
                if filekey in self.singlefiles:
    ...
    def showRedundanceFile(self, filepath):
        ...
        print(
            'the total file num:{0},the redundance file num(not including the first file):{1}'.format(
                self.allfilecount,
                self.rddfilecount))
        print('-----------------------------------------')
        for k in self.rddfiles.keys():
            for l in sorted(
                    self.rddfiles.get(k), key=itemgetter(1)):  # 按修改日期升序排列
                print(l)
            print('')
        print('------------------------------------------')
